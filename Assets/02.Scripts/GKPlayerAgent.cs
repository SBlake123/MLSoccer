...
public class GKPlayerAgent : Agent
{
    public enum Team
    {
        BLUE = 0, RED
    ...
    //플레이어의 초기 위치
    private Vector3 initPosBlue = new Vector3(-17f, 0.5f, 0.0f);
    private Vector3 initPosRed = new Vector3(+16f, 0.5f, 0.0f);
    ...
    {
        if(other.CompareTag("GKRED") && (team == Team.RED))
        {
            AddReward(0.02f);
        }

        else if(other.CompareTag("GKBLUE") && (team == Team.BLUE))
        {
            AddReward(0.02f);
        }
    }
